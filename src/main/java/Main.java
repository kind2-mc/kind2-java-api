...
public class Main {
	public static void main(String[] args) {
		...
		JKindResult result = new JKindResult("test");
	...
	public static Contract stopWatchSpec() {
		...
		c.createVarDef("on", NamedType.BOOL,
				LustreUtil.arrow(toggle, LustreUtil.or(LustreUtil.and(LustreUtil.pre(on), LustreUtil.not(toggle)),
						LustreUtil.and(LustreUtil.not(LustreUtil.pre(on)), toggle))));

		c.addAssumption(LustreUtil.not(LustreUtil.and(toggle, reset)));
		c.addGuarantee(LustreUtil.arrow(LustreUtil.implies(on, LustreUtil.equal(time, LustreUtil.integer(1))),
				LustreUtil.TRUE));
		c.addGuarantee(
				LustreUtil.arrow(LustreUtil.implies(LustreUtil.not(on), LustreUtil.equal(time, LustreUtil.integer(0))),
						LustreUtil.TRUE));
		...
		c.addMode("stopped", Arrays.asList(LustreUtil.not(reset), LustreUtil.not(on)), Collections
				.singletonList(LustreUtil.arrow(LustreUtil.TRUE, LustreUtil.equal(time, LustreUtil.pre(time)))));
	...
	public static NodeBuilder stopWatch() {
		...
		ContractBodyBuilder c = new ContractBodyBuilder();
		c.importContract("StopWatchSpec", Arrays.asList(toggle, reset), Collections.singletonList(count));

		c.addGuarantee(LustreUtil.not(LustreUtil.and(LustreUtil.modeRef("StopWatchSpec", "resetting"),
				LustreUtil.modeRef("StopWatchSpec", "running"), LustreUtil.modeRef("StopWatchSpec", "stopped"))));
